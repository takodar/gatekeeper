...
class Topic extends Forum {
    ...
    public $tags = array();
    public static $FETCH_TYPE_MOST_POPULAR = "popular";
    public static $FETCH_TYPE_LATEST = "newest";
    public static $FETCH_TYPE_MOST_COMMENTED = "commented";
    ...
    /**
     * Calls the appropriate fetch method based on type
     * @param $from
     * @param $type
     * @return array
     */
    public function getTopics($from, $type) {

        if ($type == Topic::$FETCH_TYPE_LATEST) {

            return $this->getLatestTopics($from);
        } elseif ($type == Topic::$FETCH_TYPE_MOST_POPULAR) {

            return $this->getMostPopularTopics($from);
        } else if ($type == Topic::$FETCH_TYPE_MOST_COMMENTED) {

            return $this->getMostCommentedTopics($from);
        }
    }
    ...
    public function get_topics($cat_id, $page = 1) {
        ...
        if ($page < 1)
            $page = 1; // cant make $from -ve
        $from = ($page - 1) * $num_posts;
        // username , title, topic created, no of replies,
        $qry = 'SELECT p.reputation,p.post_id,p.omessage AS message, p.post_created, r.rid, u.id, u.name AS name, u.avatar, ' . 't.topic_id, t.uid, t.no_posts, t.no_views, t.title, t.topic_created,last_post_id, ' . 't.last_post_name AS lname, t.last_post_uid AS luid, t.last_post_time AS lpost_time,' . 't.topic_status ' . 'FROM ' . PREFIX . 'codo_topics AS t ' . 'LEFT JOIN ' . PREFIX . 'codo_posts AS p ON (t.post_id=p.post_id AND p.post_status=1) ' . 'LEFT JOIN ' . PREFIX . 'codo_users AS u ON u.id=p.uid ' . 'LEFT JOIN ' . PREFIX . 'codo_user_roles AS r ON u.id=r.uid AND r.is_primary=1 ' . 'WHERE t.cat_id=' . $cat_id . ' AND (' . ' t.topic_status=' . Forum::APPROVED . ' OR' . ' t.topic_status=' . Forum::APPROVED_CLOSED . ' OR' . ' t.topic_status=' . Forum::STICKY . ' OR' . ' t.topic_status=' . Forum::STICKY_CLOSED . ' OR' . ' t.topic_status=' . Forum::STICKY_ONLY_CATEGORY . ' OR' . ' t.topic_status=' . Forum::STICKY_ONLY_CATEGORY_CLOSED . '  ) AND ' . $this->getViewTopicPermissionConditions() . 'ORDER BY t.topic_status DESC, t.last_post_time DESC LIMIT ' . $num_posts . ' OFFSET ' . $from;
    ...
    public function getTaggedTopics($tag, $from = 0) {
        ...
        // username , title, topic created, no of replies,
        $qry = 'SELECT p.reputation,p.post_id, p.omessage AS message, p.post_created, r.rid, u.id, u.name as name, u.avatar, c.cat_img, c.cat_alias,' . 't.topic_id, t.uid, t.title, t.no_posts, t.no_views, t.last_post_time, t.last_post_uid, ' . 't.last_post_name AS last_post_name, t.topic_created, t.cat_id, last_post_id,t.topic_status ' . 'FROM codo_topics AS t INNER JOIN codo_tags as g ON g.topic_id=t.topic_id ' . 'LEFT JOIN codo_posts AS p ON (t.post_id=p.post_id AND p.post_status=1)' . 'LEFT JOIN codo_users AS u ON u.id=p.uid ' . 'LEFT JOIN codo_categories AS c ON c.cat_id=t.cat_id ' . 'LEFT JOIN ' . PREFIX . 'codo_user_roles AS r ON u.id=r.uid AND r.is_primary=1 ' . ' WHERE (' . ' t.topic_status=' . Forum::APPROVED . ' OR' . ' t.topic_status=' . Forum::APPROVED_CLOSED . ' OR' . ' t.topic_status=' . Forum::STICKY . ' OR' . ' t.topic_status=' . Forum::STICKY_CLOSED . ' OR' . ' t.topic_status=' . Forum::STICKY_ONLY_CATEGORY . ' OR' . ' t.topic_status=' . Forum::STICKY_ONLY_CATEGORY_CLOSED . '  ) AND ' . $this->getViewTopicPermissionConditions() . ' AND g.tag_name=' . $this->db->quote($tag) . ' ORDER BY t.last_post_time DESC LIMIT  ' . $num_posts . ' OFFSET ' . $from;
    ...
    public function getTaggedTopicsCount($tag) {
        $status = array(
            Forum::APPROVED,
            Forum::APPROVED_CLOSED,
            Forum::STICKY,
            Forum::STICKY_CLOSED,
            Forum::STICKY_ONLY_CATEGORY,
            Forum::STICKY_ONLY_CATEGORY_CLOSED
        );

        $qry = 'SELECT COUNT(t.topic_id) AS num_tagged FROM ' . PREFIX . 'codo_topics AS t' . ' INNER JOIN ' . PREFIX . 'codo_tags AS g ON g.topic_id=t.topic_id ' . ' WHERE t.topic_status IN (' . implode($status) . ') AND g.tag_name=' . $this->db->quote($tag);
    ...
    // TODO: make this a little more reusable
    private function getLatestTopics($from = 0) {
        $_topics = array();
        // $t = microtime(true);
        $num_posts = \CODOF\Util::get_opt("num_posts_all_topics");

        $qry = 'SELECT  p.reputation,p.post_id, p.omessage AS message, p.post_created, r.rid, u.id, ' . 'u.name as name, u.avatar, c.cat_id, c.cat_img, c.cat_alias, ' . 't.topic_id, t.uid, t.title, t.no_posts, t.no_views, ' . 't.last_post_time, t.last_post_uid, last_post_id, t.topic_status, ' . 't.last_post_name AS last_post_name, t.topic_created ' . 'FROM codo_topics AS t ' . 'LEFT JOIN codo_posts AS p ON (t.post_id=p.post_id AND p.post_status=1)' . 'LEFT JOIN codo_users AS u ON u.id=p.uid ' . 'LEFT JOIN codo_categories AS c ON c.cat_id=t.cat_id ' . 'LEFT JOIN ' . PREFIX . 'codo_user_roles AS r ON u.id=r.uid AND r.is_primary=1 ' . 'WHERE ( ' . ' t.topic_status=' . Forum::APPROVED . ' OR' . ' t.topic_status=' . Forum::APPROVED_CLOSED . ' OR' . ' t.topic_status=' . Forum::STICKY . ' OR' . ' t.topic_status=' . Forum::STICKY_CLOSED . ' ) AND ' . $this->getViewTopicPermissionConditions() . 'ORDER BY t.topic_status DESC, t.last_post_time DESC ' . 'LIMIT  ' . $num_posts . ' OFFSET ' . $from;
    ...
    /**
     * Gets the most popular topics based on post reputation
     * @param int $from
     * @return array
     */
    private function getMostPopularTopics($from = 0) {
        $_topics = array();
        // $t = microtime(true);
        $num_posts = \CODOF\Util::get_opt("num_posts_all_topics");

        $qry = 'SELECT  p.post_id, p.omessage AS message, p.post_created, r.rid, u.id, '
                ...
                . 'ORDER BY p.reputation DESC, t.no_posts DESC, t.last_post_time DESC '
    ...
    /**
     * Gets the most commented topics based on reply count of every topic
     * @param int $from
     * @return array
     */
    private function getMostCommentedTopics($from = 0) {
        $_topics = array();
        // $t = microtime(true);
        $num_posts = \CODOF\Util::get_opt("num_posts_all_topics");
        ...
        $qry = 'SELECT  p.post_id, p.omessage AS message, p.post_created, r.rid, u.id, ' .
                ...
                't.topic_id, t.uid, t.title, t.no_posts, t.no_views, ' .
                't.last_post_time, t.last_post_uid, last_post_id, t.topic_status, ' .
                't.last_post_name AS last_post_name, t.topic_created, p.reputation ' .
                'FROM codo_topics AS t ' .
                'LEFT JOIN codo_posts AS p ON (t.post_id=p.post_id AND p.post_status=1)' .
                'LEFT JOIN codo_users AS u ON u.id=p.uid ' .
                'LEFT JOIN codo_categories AS c ON c.cat_id=t.cat_id ' .
                ...
                ' t.topic_status=' . Forum::STICKY_CLOSED . ' ) AND ' .
                $this->getViewTopicPermissionConditions() .
                'ORDER BY t.no_posts DESC, t.last_post_time DESC ' .
                'LIMIT  ' . $num_posts . ' OFFSET ' . $from;
    ...
    public function getViewTopicPermissionConditions() {
        ...
        $conditions = ' ' . 'EXISTS (SELECT 1 FROM codo_permissions AS permission  ' . ' WHERE  permission.rid IN (' . $rids . ') ' . ' AND ' . '  (' . '    (' . '      permission.cid = t.cat_id' . '      AND permission.tid=0 ' . '    )' . '    OR ' . '    permission.tid=t.topic_id' . '  ) ' . ' AND permission.granted=1 ' . ' AND ' . '  (' . '    permission.permission=\'view all topics\' OR ' . '    (permission.permission=\'view my topics\' AND t.uid=' . $user->id . ') ' . '  ) ' . ' )';
    ...
    public function ins_topic($catid, $title, $needsModeration, $topic_status = Forum::APPROVED) {
        ...
        $qry = "INSERT INTO codo_topics (title, cat_id, uid, topic_created, last_post_time,topic_status) " . "VALUES(:title, :catid, :uid, :created_time, :post_time, :topic_status)";
     ...
     * @param type $pollOptions
     */
    public function addPollToTopic($tid, $pollTitle, $pollData) {

        $pollData["canRecast"] = $pollData['canRecast'] == 'yes' ? 1 : 0;
        $pollData["viewResultWithoutVote"] = $pollData['viewResultWithoutVote'] == 'yes' ? 1 : 0;
        $pollData['endTime'] = $pollData['endTime'] != null ? strtotime($pollData['endTime']) : -1;

        Poll::add($tid, $pollTitle, $pollData);
    }
     ...
     * @return type
     */
    public function updatePollIfExists($tid, $pollTitle, $pollData, $hasPoll) {
        ...
        if ($poll) {
            ...
            foreach ($poll['options'] as $option) {
                ...
                if (!in_array($optionName, $pollData['options'])) {

                    $deletedOptions[] = $optionName;
                }

                $existingOptions[] = $optionName; //reusing loop to create simple array
            }
            ...
            if (count($deletedOptions) > 0) {
                Poll::deleteOptions($poll['id'], $deletedOptions);
            }
            if (count($newOptions) > 0) {
                Poll::addOptions($poll['id'], $newOptions);
            }
            Poll::updatePoll($poll['id'], $pollTitle, $pollData);
        }
    }
    ...
    public function get_topic_info($tid) {
        ...
        $qry = "SELECT t.redirect_to,t.topic_id,t.post_id, t.no_posts, t.no_views,t.uid," . "t.title, c.cat_name,t.post_id, c.cat_alias, c.cat_id," . "t.topic_created, t.topic_updated, t.topic_status " . "FROM codo_topics AS t " . "INNER JOIN codo_categories AS c ON c.cat_id=t.cat_id " . "WHERE t.topic_id=$tid AND t.topic_status<>0 LIMIT 1 OFFSET 0";
    ...
    public function update_last_post_details($options) {
        $qry = 'UPDATE ' . PREFIX . 'codo_topics SET last_post_id=:pid, ' . ' last_post_uid=:uid, last_post_name=:name, last_post_time=:time ' . 'WHERE topic_id=:tid';
    ...
    /**
     *
     * Gets number of posts present in given topic(topic id)
     *
     * @param type $tid
     * @return type
     */
    public function getNumPosts($tid) {
        $cid = (int) $tid;
        $qry = "SELECT no_posts FROM " . PREFIX . "codo_topics WHERE topic_id=$tid";
        $stmt = $this->db->query($qry);
        $res = $stmt->fetch();
    ...
    public function get_total_num_topics() {
        $qry = "SELECT COUNT(t.topic_id) AS total_num_topics FROM " . PREFIX . "codo_topics AS t" . " LEFT JOIN " . PREFIX . "codo_categories AS c ON c.cat_id=t.cat_id" . " WHERE t.topic_status <> 0 AND " . $this->getViewTopicPermissionConditions();
    ...
    protected function set_status($id, $option) {
        ...
        $qry = 'UPDATE ' . PREFIX . 'codo_topics SET topic_status=' . $status [$option] . ' ' . 'WHERE topic_id=' . $tid;
        $this->db->query($qry);
    }
    ...
    public function getAllTags($tids) {
        ...
        $qry = 'SELECT g.tag_name,g.topic_id FROM ' . PREFIX . 'codo_tags AS g ' . 'WHERE g.topic_id IN (' . implode(",", $tids) . ')';
        ...
        if ($res) {
            ...
            foreach ($_tags as $tag) {
                ...
                $tags [$tag ['topic_id']] [] = array(
                    'tag' => $tag ['tag_name'],
                    'safe_tag' => urlencode($tag ['tag_name'])
                );
    ...
    public function gen_topic_arr_all_topics($topics, $search = false) {
        ...
        foreach ($topics as $topic) {
            ...
            $_topics [$i] ['can_report_topic'] = $user->can('report topics', $topic ['cat_id']);
    ...
    public function canEditTopic($tuid, $cid, $tid) {
        ...
        return $tuid == $user->id && $user->canAny(array(
                ...
                $tuid != $user->id && $user->can('edit all topics', $cid, $tid);
    ...
    public function canReplyTopic($tuid, $cid, $tid) {
        ...
        return $tuid == $user->id || // &&
                ...
                $tuid != $user->id && $user->can('reply to all topics', $cid, $tid);
    ...
    public function canDeleteTopic($tuid, $cid, $tid) {
        ...
        return $tuid == $user->id && $user->canAny(array(
                ...
                // can i reply to my own topic ?
                $tuid != $user->id && $user->can('delete all topics', $cid, $tid);
        // can i reply to others' topic ?
    }
