...
' : '').''.(($cx['funcs']['ifvar']($cx, ((isset($in['can_see_history']) && is_array($in)) ? $in['can_see_history'] : null))) ? '                    <li id="codo_posts_history_'.((isset($in['post_id']) && is_array($in)) ? $in['post_id'] : null).'" class="codo_posts_history"
                        role="presentation"><a role="menuitem" tabindex="-1">
                            <i class="glyphicon glyphicon-time"></i> '.$cx['funcs']['ch']($cx, 'i18n', array(array('History'),array()), 'raw').'
                        </a></li>
' : '').'
                </ul>
            </div>
...
' : '').'').'    </div>
    ...
    <div class="codo_posts_user_info">
        ...
        <div class="codo_posts_post_name">
            <a href="'.$cx['funcs']['ch']($cx, 'const', array(array('RURI'),array()), 'raw').'user/profile/'.htmlentities((string)((isset($in['id']) && is_array($in)) ? $in['id'] : null), ENT_QUOTES, 'UTF-8').'"><span class="role_'.((isset($in['role']) && is_array($in)) ? $in['role'] : null).'" style="font-weight:bold;color:#616161;">'.((isset($in['name']) && is_array($in)) ? $in['name'] : null).'</span></a>
        </div>
...
'.(($cx['funcs']['ifvar']($cx, ((isset($in['post_modified']) && is_array($in)) ? $in['post_modified'] : null))) ? '            <div class="codo_last_edit">'.$cx['funcs']['ch']($cx, 'i18n', array(array('edited'),array()), 'raw').' '.htmlentities((string)((isset($in['post_modified']) && is_array($in)) ? $in['post_modified'] : null), ENT_QUOTES, 'UTF-8').'</div>
' : '').'        
'.(($cx['funcs']['ifvar']($cx, ((isset($in['can_move_topic']) && is_array($in)) ? $in['can_move_topic'] : null))) ? '            <input type="checkbox" id="codo_posts_select_'.((isset($in['post_id']) && is_array($in)) ? $in['post_id'] : null).'" class="codo_posts_select_post"/>
' : '').''.(($cx['funcs']['ifvar']($cx, ((isset($in['can_move_post']) && is_array($in)) ? $in['can_move_post'] : null))) ? '            <input type="checkbox" id="codo_posts_select_'.((isset($in['post_id']) && is_array($in)) ? $in['post_id'] : null).'" class="codo_posts_select_post"/>
' : '').'        
'.(($cx['funcs']['ifvar']($cx, ((isset($in['can_rep']) && is_array($in)) ? $in['can_rep'] : null))) ? '        <div class="codo_posts_post_reputation" id="codo_posts_rep_'.((isset($in['post_id']) && is_array($in)) ? $in['post_id'] : null).'">
            <div class="codo_btn codo_btn_danger codo_rep_up_btn codo_tooltip" title="'.$cx['funcs']['ch']($cx, 'i18n', array(array('vote up'),array()), 'encq').'"><i class="icon-thumbs-up"></i></div>
            <div class="codo_reputation_points codo_tooltip '.(($cx['funcs']['ifvar']($cx, ((isset($in['is_neg_rep']) && is_array($in)) ? $in['is_neg_rep'] : null))) ? 'codo_reputation_negative' : '').'" data-placement="right" title="'.$cx['funcs']['ch']($cx, 'i18n', array(array('reputation points'),array()), 'encq').'">'.htmlentities((string)((isset($in['reputation']) && is_array($in)) ? $in['reputation'] : null), ENT_QUOTES, 'UTF-8').'</div>
            <div class="codo_btn codo_btn_danger codo_rep_down_btn codo_tooltip" title="'.$cx['funcs']['ch']($cx, 'i18n', array(array('vote down'),array()), 'encq').'" data-placement="bottom"><i class="icon-thumbs-down"></i></div>
        </div>
' : '').'
    </div>  
