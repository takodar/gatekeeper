...
if($_POST) //Post Data received from product list page.
{
	...
	if($GrandTotal <= 0)
	{
	    $returnVal = register($_SESSION['order'], $_SESSION['password'], $_SESSION['pin'], $_SESSION['pin_hint'], $_SESSION['master_pin'], $GrandTotal);
	    if($returnVal != 'ok')
	    {
            echo '<h1 class="errorMessage">';
                print_r('The following problem was encountered: ');
                print_r($returnVal);
            echo '</h1>';
            echo '<h1 class="errorMessage">';
                print_r('Please email support@jigging.unh.edu with the "user name" will get it resolved!');
            echo '</h1>';
        }
       ...
       $master_pin = urlencode($master_pin);
		...
		if ($result4) {
			...
			if($obj->{'status'} != 'ok')
            {
                ...
                echo '</h1>';
                echo '<h1 class="errorMessage">';
                    print_r('Please email support@authentry.com with the "user name" will get it resolved!');
                echo '</h1>';
            }
		}	
		print_r($datares);
